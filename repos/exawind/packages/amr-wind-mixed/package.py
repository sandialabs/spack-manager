...
class AmrWindMixed(CMakePackage, CudaPackage, ROCmPackage):
    ...
    depends_on('hypre-mixed~int64@2.20.0:', when='+hypre')
    depends_on('hypre-mixed+mpi', when='+mpi')
    for arch in CudaPackage.cuda_arch_values:
        depends_on('hypre-mixed+unified-memory+cuda cuda_arch=%s' % arch,
                   when='+hypre+cuda cuda_arch=%s' % arch)
    for arch in ROCmPackage.amdgpu_targets:
        depends_on('hypre-mixed~int64+rocm amdgpu_target=%s' % arch,
                   when='+hypre+rocm amdgpu_target=%s' % arch)
