...
parser = argparse.ArgumentParser("Run a test of the developer snapshot workflow")
parser.add_argument("--view_name", required=False, help="view to use for the developer")
parser.add_argument("--snap_name", required=False, help="directory name for the snapshot")
...
parser.set_defaults(view_name="default", snap_name="test", dev_name="test_external", num_threads=1)
args = parser.parse_args()

# set up the snapshot
print("Create Snapshot")
args = snapshot_creator.parse(
    ["--use_develop", "--modules", "--name", args.snap_name, "--num_threads", args.num_threads]
)
...
command(manager, "external", snapshot_path, "-v", args.view_name, "--blacklist", "nalu-wind")
