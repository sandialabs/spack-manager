...
def variant_factory(spec_string):
    if 'nalu-wind' in spec_string:
        ...
        manager('develop', '-rb', 'https://github.com/trilinos/trilinos', branch, 'trilinos@{b}'.format(b=branch))
...
def create_test_env(args):
    if args.test_nalu:
        manager('create-env', '-d', args.directory, '-s', 'nalu-wind-nightly'+args.nalu_wind_vars)
        with env.Environment(args.directory):
            ...
            manager('develop', '-rb', 'https://github.com/Exawind/nalu-wind', 'master', test_package)
