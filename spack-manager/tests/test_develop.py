...
class TestSpackManagerDevelop(object):
    @patch("manager.cmd.manager_cmds.develop.spack_develop.develop")
    ...
    @patch("manager.cmd.manager_cmds.develop.spack_develop.develop")
    @patch("manager.cmd.manager_cmds.develop.git_clone")
    @patch("manager.cmd.manager_cmds.develop.git_remote_add")
    ...
    def test_spackManagerExtensionArgsLeadToGitCalls(
        self, mock_git_remote, mock_git_clone, mock_develop, all_branches, shallow, add_remote
    ):
        env("create", "test")
        with ev.read("test") as e:
            ...
            mock_develop.assert_called_once()
            mock_git_clone.assert_called_once_with(branch, repo, path, shallow, all_branches)
            if add_remote:
